...
if (!file) {
  console.error('Usage: node scripts/deck_validate.mjs <index.html> [--allow-experimental]');
  process.exit(2);
}
...
slides.forEach((slide) => {
  ...
  if (!layout) {
    ...
    errors.push(`Slide ${slide.idx}: data-layout="${layout}" is not registered in the layout lock (see references/22-style-minimal.md).`);
  }
